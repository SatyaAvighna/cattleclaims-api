...
class C_role_items extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('role_items');
	}
	public function index()
	{
		$arry = array();
		$role_items = $this->role_items->getrole_itemss();
		$arry['status'] = "success";
		$arry['message'] = "Entry retrieved successfully.";	
		$arry['list'] = $role_items;
	...
	public function add()
	{
		...
		$result = $this->role_items->insertrole_itemsById($data);	
		if($result)
		{
			$arry['status'] = "success";
	...
	public function retrive()
	{
		...
		if(!empty($data['role_items_Id']))
		{
			$this->encryption->initialize(array('driver' => 'openssl','cipher' => 'aes-256','mode' => 'ctr'));
			$data['sId'] =  $this->encryption->decrypt($data['sId']);
			$lists = $this->role_items->getrole_itemsById($data);	
			$arry['status'] = "success";
	...
	public function update()
	{
		...
		if(!empty($data['role_items_Id']))
		{
			...
			$this->encryption->initialize(array('driver' => 'openssl','cipher' => 'aes-256','mode' => 'ctr'));
			$data['sId'] =  $this->encryption->decrypt($data['sId']);
			$result = $this->role_items->updaterole_itemsById($data);	
			if($result)
			{
				$arry['status'] = "success";
	...
	public function delete()
	{
		...
		if(!empty($data['role_items_Id']))
		{
			...
			$this->encryption->initialize(array('driver' => 'openssl','cipher' => 'aes-256','mode' => 'ctr'));
			$data['sId'] =  $this->encryption->decrypt($data['sId']);
			$result = $this->role_items->deleterole_itemsById($data);	
			if($result)
			{
				$arry['status'] = "success";
