...
class C_suminsured extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('suminsured');
	}
	public function index()
	{
		$arry = array();
		$suminsured = $this->suminsured->getsuminsureds();
		$arry['status'] = "success";
		$arry['message'] = "Entry retrieved successfully.";	
		$arry['list'] = $suminsured;
	...
	public function add()
	{
		...
		$result = $this->suminsured->insertsuminsuredById($data);	
		if($result)
		{
			$arry['status'] = "success";
	...
	public function retrive()
	{
		...
		if(!empty($data['siId']))
		{
			$this->encryption->initialize(array('driver' => 'openssl','cipher' => 'aes-256','mode' => 'ctr'));
			$data['sId'] =  $this->encryption->decrypt($data['sId']);
			$lists = $this->suminsured->getsuminsuredById($data);	
			$arry['status'] = "success";
	...
	public function update()
	{
		...
		if(!empty($data['siId']))
		{
			...
			$this->encryption->initialize(array('driver' => 'openssl','cipher' => 'aes-256','mode' => 'ctr'));
			$data['sId'] =  $this->encryption->decrypt($data['sId']);
			$result = $this->suminsured->updatesuminsuredById($data);	
			if($result)
			{
				$arry['status'] = "success";
	...
	public function delete()
	{
		...
		if(!empty($data['siId']))
		{
			...
			$this->encryption->initialize(array('driver' => 'openssl','cipher' => 'aes-256','mode' => 'ctr'));
			$data['sId'] =  $this->encryption->decrypt($data['sId']);
			$result = $this->suminsured->deletesuminsuredById($data);	
			if($result)
			{
				$arry['status'] = "success";
