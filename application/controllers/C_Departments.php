...
class C_departments extends CI_Controller {
	...
	{
		$arry = array();
		$departments = $this->departments->getdepartmentss();
		$arry['status'] = "success";
		$arry['message'] = "Entry retrieved successfully.";	
		$arry['list'] = $departments;
	...
	public function add()
	{
		...
		$result = $this->departments->insertdepartmentsById($data);	
		if($result)
		{
			$arry['status'] = "success";
	...
	public function retrive()
	{
		...
		if(!empty($data['departments_Id']))
		{
			$this->encryption->initialize(array('driver' => 'openssl','cipher' => 'aes-256','mode' => 'ctr'));
			$data['sId'] =  $this->encryption->decrypt($data['sId']);
			$lists = $this->departments->getdepartmentsById($data);	
			$arry['status'] = "success";
	...
	public function update()
	{
		...
		if(!empty($data['departments_Id']))
		{
			...
			$this->encryption->initialize(array('driver' => 'openssl','cipher' => 'aes-256','mode' => 'ctr'));
			$data['sId'] =  $this->encryption->decrypt($data['sId']);
			$result = $this->departments->updatedepartmentsById($data);	
			if($result)
			{
				$arry['status'] = "success";
	...
	public function delete()
	{
		...
		if(!empty($data['departments_Id']))
		{
			...
			$this->encryption->initialize(array('driver' => 'openssl','cipher' => 'aes-256','mode' => 'ctr'));
			$data['sId'] =  $this->encryption->decrypt($data['sId']);
			$result = $this->departments->deletedepartmentsById($data);	
			if($result)
			{
				$arry['status'] = "success";
