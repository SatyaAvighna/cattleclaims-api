...
class animaltype extends CI_Controller {
	...
	public function add()
	{
		...
		$result = $this->animaltype->insertanimaltypeById($data);	
		if($result)
		{
			$arry['status'] = "success";
	...
	public function update()
	{
		...
		if(!empty($data['animaltype_Id']))
		{
			...
			$this->encryption->initialize(array('driver' => 'openssl','cipher' => 'aes-256','mode' => 'ctr'));
			$data['sId'] =  $this->encryption->decrypt($data['sId']);
			$result = $this->animaltype->updateanimaltypeById($data);	
			if($result)
			{
				$arry['status'] = "success";
	...
	public function delete()
	{
		...
		if(!empty($data['animaltype_Id']))
		{
			...
			$this->encryption->initialize(array('driver' => 'openssl','cipher' => 'aes-256','mode' => 'ctr'));
			$data['sId'] =  $this->encryption->decrypt($data['sId']);
			$result = $this->animaltype->deleteanimaltypeById($data);	
			if($result)
			{
				$arry['status'] = "success";
