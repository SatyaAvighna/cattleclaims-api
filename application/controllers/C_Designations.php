...
class C_designations extends CI_Controller {
	...
	{
		$arry = array();
		$designations = $this->designations->getdesignationss();
		$arry['status'] = "success";
		$arry['message'] = "Entry retrieved successfully.";	
		$arry['list'] = $designations;
	...
	public function add()
	{
		...
		$result = $this->designations->insertdesignationsById($data);	
		if($result)
		{
			$arry['status'] = "success";
	...
	public function retrive()
	{
		...
		if(!empty($data['designations_Id']))
		{
			$this->encryption->initialize(array('driver' => 'openssl','cipher' => 'aes-256','mode' => 'ctr'));
			$data['sId'] =  $this->encryption->decrypt($data['sId']);
			$lists = $this->designations->getdesignationsById($data);	
			$arry['status'] = "success";
	...
	public function update()
	{
		...
		if(!empty($data['designations_Id']))
		{
			...
			$this->encryption->initialize(array('driver' => 'openssl','cipher' => 'aes-256','mode' => 'ctr'));
			$data['sId'] =  $this->encryption->decrypt($data['sId']);
			$result = $this->designations->updatedesignationsById($data);	
			if($result)
			{
				$arry['status'] = "success";
	...
	public function delete()
	{
		...
		if(!empty($data['designations_Id']))
		{
			...
			$this->encryption->initialize(array('driver' => 'openssl','cipher' => 'aes-256','mode' => 'ctr'));
			$data['sId'] =  $this->encryption->decrypt($data['sId']);
			$result = $this->designations->deletedesignationsById($data);	
			if($result)
			{
				$arry['status'] = "success";
