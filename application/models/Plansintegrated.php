...
{ 
	...
	public function updateplansintegratedById($req) 
	{
		...
		if(!empty($req["iD"])) $set .= "iD=".$this->db->escape($req["iD"]).",";if(!empty($req["insurerName"])) $set .= "insurerName=".$this->db->escape($req["insurerName"]).",";if(!empty($req["insurerLogo"])) $set .= "insurerLogo=".$this->db->escape($req["insurerLogo"]).",";if(!empty($req["sumInsured"])) $set .= "sumInsured=".$this->db->escape($req["sumInsured"]).",";if(!empty($req["animalType"])) $set .= "animalType=".$this->db->escape($req["animalType"]).",";if(!empty($req["breed"])) $set .= "breed=".$this->db->escape($req["breed"]).",";if(!empty($req["gender"])) $set .= "gender=".$this->db->escape($req["gender"]).",";if(!empty($req["age"])) $set .= "age=".$this->db->escape($req["age"]).",";if(!empty($req["premium"])) $set .= "premium=".$this->db->escape($req["premium"]).",";if(!empty($req["risk"])) $set .= "risk=".$this->db->escape($req["risk"]).",";if(!empty($req["dummyOne"])) $set .= "dummyOne=".$this->db->escape($req["dummyOne"]).",";if(!empty($req["dummyTwo"])) $set .= "dummyTwo=".$this->db->escape($req["dummyTwo"]).",";if(!empty($req["dummyThree"])) $set .= "dummyThree=".$this->db->escape($req["dummyThree"]).",";if(!empty($req["dummyFour"])) $set .= "dummyFour=".$this->db->escape($req["dummyFour"]).",";if(!empty($req["dummyFive"])) $set .= "dummyFive=".$this->db->escape($req["dummyFive"]).",";if(!empty($req["sId"])) $set .= "updatedBy=".$this->db->escape($req["sId"]).",";;
	...
	{
		$status = false;
		$query =  $this->db->query("INSERT INTO plansintegrated(iD,insurerName,insurerLogo,sumInsured,animalType,breed,gender,age,premium,risk,dummyOne,dummyTwo,dummyThree,dummyFour,dummyFive,createdBy) VALUES (".$this->db->escape($req["iD"]).",".$this->db->escape($req["insurerName"]).",".$this->db->escape($req["insurerLogo"]).",".$this->db->escape($req["sumInsured"]).",".$this->db->escape($req["animalType"]).",".$this->db->escape($req["breed"]).",".$this->db->escape($req["gender"]).",".$this->db->escape($req["age"]).",".$this->db->escape($req["premium"]).",".$this->db->escape($req["risk"]).",".$this->db->escape($req["dummyOne"]).",".$this->db->escape($req["dummyTwo"]).",".$this->db->escape($req["dummyThree"]).",".$this->db->escape($req["dummyFour"]).",".$this->db->escape($req["dummyFive"]).",".$this->db->escape($req["sId"]).")");
		if($this->db->affected_rows()>0)
		{
			//echo "i";
			$this->mc->memcached->delete($this->config->config['cKey']."_plansintegrated");
